...
summary(centered)
m = glmer(reducedRelativizer ~ RCType + HeadSpecificity +  Adjacency + Type.of.Antecedent  + logRClength + Speaker_sex + Speaker_Age + Speaker_education + (1|Speaker_ID),data=centered,family="binomial")
summary(m)

m = glmer(reducedRelativizer ~ RCType*(cHeadSpecificity +  cAdjacency  + clogRClength + cSpeaker_sex + cSpeaker_Age + cSpeaker_education) + (1|Speaker_ID),data=centered,family="binomial")
summary(m)
